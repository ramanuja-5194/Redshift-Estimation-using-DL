...
class AstroDenseNet(nn.Module):
    def __init__(self):
        ...
        # Load standard DenseNet121 (Untrained)
        ...
        self.densenet.classifier = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(256, 1) # Output single continuous Redshift value
        )
...
class NormalizedL1Loss(nn.Module):
    """
    Directly optimizes the relative error: |z_pred - z_true| / (1 + z_true)
    """
    def __init__(self):
...
def calculate_astronomy_metrics(y_true, y_pred):
    ...
    outlier_rate_15 = np.mean(np.abs(dz_norm) > 0.15) * 100 # Standard astronomy outlier cutoff
...
plt.subplot(1, 2, 1)
plt.plot(train_losses, label='Train Loss (Norm L1)')
plt.plot(val_losses, label='Val Loss (Norm L1)')
plt.xlabel('Epochs')
plt.ylabel('Normalized L1 Loss')
plt.legend()
...
plt.tight_layout()
plt.savefig("densenet_model_results.png")
plt.show()