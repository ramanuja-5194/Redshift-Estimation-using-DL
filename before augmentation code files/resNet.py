...
# 3. DEFINE MODEL: STANDARD RESNET-18
class AstroResNet(nn.Module):
    def __init__(self):
        super(AstroResNet, self).__init__()
        
        # Load standard ResNet-18 (Untrained, since weights are for 3-channel RGB)
        self.resnet = models.resnet18(weights=None)
        
        # SURGERY STEP 1: Modify input layer for 5 Channels (u, g, r, i, z)
        ...
        self.resnet.fc = nn.Sequential(
            ...
            nn.Linear(num_features, 1) # Output 1 value (Redshift)
        )

    def forward(self, x):
        return self.resnet(x)

model = AstroResNet().to(DEVICE)

# Using Huber Loss to prevent catastrophic outliers from ruining gradients
...
for epoch in range(EPOCHS):
    ...
    print(f"Epoch {epoch+1:02d}/{EPOCHS} | Train Loss: {train_loss:.6f} | Val Loss: {val_loss:.6f} {status}")
